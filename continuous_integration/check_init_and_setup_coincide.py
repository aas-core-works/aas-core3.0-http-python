...
"""Check that the distribution and aas_core3_http/__init__.py are in sync."""
import os
import pathlib
import subprocess
import sys
...
def main() -> int:
    ...
    if setup_py_map["version"] != aas_core3_http.__version__:
        print(
            f"The version in the setup.py is {setup_py_map['version']}, "
            f"while the version in aas_core3_http/__init__.py is: "
            f"{aas_core3_http.__version__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["author"] != aas_core3_http.__author__:
        print(
            f"The author in the setup.py is {setup_py_map['author']}, "
            f"while the author in aas_core3_http/__init__.py is: "
            f"{aas_core3_http.__author__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["license"] != aas_core3_http.__license__:
        print(
            f"The license in the setup.py is {setup_py_map['license']}, "
            f"while the license in aas_core3_http/__init__.py is: "
            f"{aas_core3_http.__license__}",
            file=sys.stderr,
        )
        success = False

    if setup_py_map["description"] != aas_core3_http.__doc__:
        print(
            f"The description in the setup.py is {setup_py_map['description']}, "
            f"while the description in aas_core3_http/__init__.py is: "
            f"{aas_core3_http.__doc__}",
            file=sys.stderr,
        )
        success = False
    ...
    if status_classifier is None:
        ...
        if expected_status_in_init != aas_core3_http.__status__:
            print(
                f"Expected status {expected_status_in_init} "
                f"according to setup.py in aas_core3_http/__init__.py, "
                f"but found: {aas_core3_http.__status__}"
            )
            success = False
