...
Here is an example how to verify an instance of :py:class:`aas_core3_http.types.Extension`:

.. code-block::

    import aas_core3_http.types as aas_types
    import aas_core3_http.verification as aas_verification
...
class _Transformer(aas_types.AbstractTransformer[Iterator[Error]]):
    ...
    def transform_paged_result_paging_metadata(
        ...
        # No verification has been defined for PagedResultPagingMetadata.
        return
        # For this uncommon return-yield construction, see:
        # https://stackoverflow.com/questions/13243766/how-to-define-an-empty-generator-function
        # noinspection PyUnreachableCode
    ...
    # noinspection PyMethodMayBeStatic
    def transform_paged_result(self, that: aas_types.PagedResult) -> Iterator[Error]:
    ...
    def transform_get_asset_administration_shell_result(
        ...
        for i, an_item in enumerate(that.result):
            for error in self.transform(an_item):
                error.path._prepend(IndexSegment(that.result, i))
                error.path._prepend(PropertySegment(that, "result"))
                yield error
    ...
    # noinspection PyMethodMayBeStatic
    def transform_message(self, that: aas_types.Message) -> Iterator[Error]:
        if not ((1 <= len(that.correlation_id) and len(that.correlation_id) <= 128)):
            yield Error("Correlation ID must be between 1 and 128 characters long.")

        if not ((1 <= len(that.code) and len(that.code) <= 32)):
            yield Error("Code must be between 1 and 32 characters long.")

        for error in verify_date_time(that.timestamp):
            error.path._prepend(PropertySegment(that, "timestamp"))
            yield error

    # noinspection PyMethodMayBeStatic
    def transform_result(self, that: aas_types.Result) -> Iterator[Error]:
        if not (len(that.messages) >= 1):
            yield Error("At least one message must be specified.")

        for i, an_item in enumerate(that.messages):
            for error in self.transform(an_item):
                error.path._prepend(IndexSegment(that.messages, i))
                error.path._prepend(PropertySegment(that, "messages"))
                yield error
...
def verify_date_time(that: str) -> Iterator[Error]:
    """Verify the constraints of :paramref:`that`."""
    if not matches_xs_date_time_utc(that):
        yield Error("The value must match the pattern of xs:dateTime.")

    if not is_xs_date_time(that):
        yield Error("The value must represent a valid xs:dateTime.")
