...
class _SetterForPagedResultPagingMetadata:
    ...
    def set_cursor_from_jsonable(self, jsonable: Jsonable) -> None:
        """
        Parse :paramref:`jsonable` as the value of :py:attr:`~cursor`.

        :param jsonable: input to be parsed
        """
        self.cursor = _str_from_jsonable(jsonable)
...
def paged_result_paging_metadata_from_jsonable(
    ...
    for key, jsonable_value in jsonable.items():
        setter_method = _SETTER_MAP_FOR_PAGED_RESULT_PAGING_METADATA.get(key)
        if setter_method is None:
            raise DeserializationException(f"Unexpected property: {key}")
    ...
    return aas_types.PagedResultPagingMetadata(setter.cursor)


def paged_result_from_jsonable(jsonable: Jsonable) -> aas_types.PagedResult:
    """
...
class _SetterForPagedResult:
    ...
    def set_paging_metadata_from_jsonable(self, jsonable: Jsonable) -> None:
        """
        Parse :paramref:`jsonable` as the value of :py:attr:`~paging_metadata`.

        :param jsonable: input to be parsed
        """
        self.paging_metadata = paged_result_paging_metadata_from_jsonable(jsonable)
...
def _paged_result_from_jsonable_without_dispatch(
    ...
    for key, jsonable_value in jsonable.items():
        setter_method = _SETTER_MAP_FOR_PAGED_RESULT.get(key)
        if setter_method is None:
            raise DeserializationException(f"Unexpected property: {key}")
...
class _SetterForGetAssetAdministrationShellsResult:
    ...
    def set_result_from_jsonable(self, jsonable: Jsonable) -> None:
        """
...
def get_submodel_result_from_jsonable(
    ...
    for key, jsonable_value in jsonable.items():
        setter_method = _SETTER_MAP_FOR_GET_SUBMODEL_RESULT.get(key)
        if setter_method is None:
            raise DeserializationException(f"Unexpected property: {key}")
...
_SETTER_MAP_FOR_PAGED_RESULT_PAGING_METADATA: Mapping[
    str, Callable[[_SetterForPagedResultPagingMetadata, Jsonable], None]
] = {
    "cursor": _SetterForPagedResultPagingMetadata.set_cursor_from_jsonable,
    "modelType": _SetterForPagedResultPagingMetadata.ignore,
}
...
_SETTER_MAP_FOR_PAGED_RESULT: Mapping[
    str, Callable[[_SetterForPagedResult, Jsonable], None]
] = {
    "pagingMetadata": _SetterForPagedResult.set_paging_metadata_from_jsonable,
    "modelType": _SetterForPagedResult.ignore,
}
...
_SETTER_MAP_FOR_GET_SUBMODEL_RESULT: Mapping[
    str, Callable[[_SetterForGetSubmodelResult, Jsonable], None]
] = {
...
class _Serializer(aas_types.AbstractTransformer[MutableJsonable]):
    ...
    def transform_paged_result(self, that: aas_types.PagedResult) -> MutableJsonable:
        """Serialize :paramref:`that` to a JSON-able representation."""
        jsonable: MutableMapping[str, MutableJsonable] = dict()
